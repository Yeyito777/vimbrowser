...
- [x] `t` — command line prefilled with `:tab-focus ` from either website or
  sidebar focus
...
- [x] `:open ai <query>` / `:open tab ai <query>` — ask ChatGPT through a
  vimbrowser-only autosubmit parameter, scrub it before ChatGPT sees the URL,
  and auto-submit the composer with a shared per-engine query history

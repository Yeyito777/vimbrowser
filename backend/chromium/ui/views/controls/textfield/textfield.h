...
class VIEWS_EXPORT Textfield : public View,
 ...
 private:
  ...
  // Re-applies logical color ranges after calls that reset the RenderText base
  // color (for example theme changes). RenderText intentionally clears ranged
  // colors when SetColor() is called; Textfield keeps the caller's ranges so
  // display-only styled textfields can survive those resets without changing
  // layout or falling back to separate views.
  void ReapplyAppliedColors();
  ...
  std::vector<AppliedColorRange> applied_color_ranges_;

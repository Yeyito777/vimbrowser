...
void Textfield::ApplyColor(SkColor value, const gfx::Range& range) {
  if (!range.IsValid() || range.is_empty() || range.GetMax() > GetText().size()) {
    return;
  }
  applied_color_ranges_.push_back({value, range});
  GetRenderText()->ApplyColor(value, range);
  SchedulePaint();
}

void Textfield::ReapplyAppliedColors() {
  const size_t text_length = GetText().size();
  for (const auto& applied_color : applied_color_ranges_) {
    if (applied_color.range.IsValid() && !applied_color.range.is_empty() &&
        applied_color.range.GetMax() <= text_length) {
      GetRenderText()->ApplyColor(applied_color.color, applied_color.range);
    }
  }
}
...
float Textfield::GetCornerRadius() {
  // Vimbrowser renders its browser chrome (sidebar rows, command line and
  // autocomplete) with CEF Views textfields for native text styling. The default
  // Material textfield radius makes selected sidebar rows look like rounded
  // pills instead of terminal-like square rows. Use square textfield geometry in
  // this source-built CEF backend; individual Views can still control color and
  // borders normally.
  return 0;
}
